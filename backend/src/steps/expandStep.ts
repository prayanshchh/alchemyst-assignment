...
export default function expandStep(agenda: Agenda) {
  agenda.define('expand', async (job: Job<ExpandJobData>, done) => {
    ...
    await runInSpan('agenda.expand', { jobId, topic }, async () => {
      ...
      const body = {
        ...
        messages: [
          ...
          { role: 'user', content: `Using the following plan, come up with some question answer format data` },
          { role: 'assistant', content: gatherResult },
          { role: 'user', content: `Using the following data, create a proper research paper summary` },
        ]
      };
